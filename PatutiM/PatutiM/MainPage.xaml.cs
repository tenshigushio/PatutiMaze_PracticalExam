...
namespace PatutiM
{
    ...
    {
        SKBitmap patuti = null, box, steelbox, a_heart, d_heart;
        ...
        public MainPage()
        {
            ...
            using (Stream stream = assembly.GetManifestResourceStream("PatutiM.a_heart.png"))
            {
                a_heart = SKBitmap.Decode(stream);
                var dstInfo = new SKImageInfo(a_heart.Width, a_heart.Height);

                a_heart = a_heart.Resize(dstInfo, SKBitmapResizeMethod.Hamming);
                //targetX = apatutiX = patuti.Width;
            }

            using (Stream stream = assembly.GetManifestResourceStream("PatutiM.d_heart.png"))
            {
                d_heart = SKBitmap.Decode(stream);
                var dstInfo = new SKImageInfo(d_heart.Width, d_heart.Height);

                d_heart = d_heart.Resize(dstInfo, SKBitmapResizeMethod.Hamming);
                //targetX = apatutiX = patuti.Width;
            }
        ...
        private async void controls_Clicked(object sender, EventArgs e)
        {
        ...
        private async void canvasView_PaintSurface(object sender, SKPaintSurfaceEventArgs e)
        {
            ...
            else
            {
                ...
                if (tocheck && patutiY>1) {
                    ...
                    else if (!tocheck == mat[(int)(patutiY / patuti.Width) - 1, (int)(patutiX / patuti.Width) - 1])
                    {
                        lives -= 1;
                        tocheck = false;
                        string check = lives.ToString();
                        bool LifeCheck = await DisplayAlert("Oppssieee!!", "Lives left: " + check, "ok", "cancel");
                    ...
                    if (lives == 0)
                    {
                        bool err = await DisplayAlert("Life 0", "Game Over", "Play Again", "cancel");
                    ...
                    if (dply && patutiX == 540 && patutiY == 900)
                    {
                        dply = false;
                        bool err = await DisplayAlert("CONGRATULATIONS", "YOU WIN", "Play Again", "cancel");
